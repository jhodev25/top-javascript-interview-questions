...
In other words, `Object.is()` matches `===` for every value except signed zeros and `NaN`. Use it when those two cases need to be distinguished from the behavior of `===`.
