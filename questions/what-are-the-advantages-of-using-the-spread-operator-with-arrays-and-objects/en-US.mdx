...
The spread operator only copies the top level. Nested objects and arrays are shared by reference, so mutating a nested value affects both copies.
...
copy.nested.b = 99;
console.log(original.nested.b); // 99 (the nested object is shared)
```

For a fully independent copy, use `structuredClone(original)` or another deep-clone approach.
