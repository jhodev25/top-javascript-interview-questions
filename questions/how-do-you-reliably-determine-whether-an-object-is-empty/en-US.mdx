...
- **`hasOwnProperty` safety**: In the `for...in` example, calling `obj.hasOwnProperty(key)` directly can fail if the object has a `null` prototype or shadows `hasOwnProperty`. Prefer `Object.hasOwn(obj, key)` (Baseline across modern browsers since 2022) or `Object.prototype.hasOwnProperty.call(obj, key)`.
