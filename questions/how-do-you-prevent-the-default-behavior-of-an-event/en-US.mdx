...
- Calling `preventDefault` has no effect on listeners added with `{ passive: true }`. Browsers treat some events like `touchstart` and `wheel` as passive by default to improve scrolling performance, so you must explicitly pass `{ passive: false }` if you need to prevent their default behavior.
- You can check `event.cancelable` to see whether an event's default action can be prevented, and `event.defaultPrevented` to check if `preventDefault` has already been called.
